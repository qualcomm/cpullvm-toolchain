...
# === Resolve VS (vcvarsall) ===
$VS_INSTALL = & "$env:ProgramFiles(x86)\Microsoft Visual Studio\Installer\vswhere.exe" -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath
if (-not $VS_INSTALL) { Write-Error "*** ERROR: Visual Studio not found ***"; exit 1 }
$VCVARSALL = "$VS_INSTALL\VC\Auxiliary\Build\vcvarsall.bat"
if (-not (Test-Path $VCVARSALL)) { Write-Error "*** ERROR: vcvarsall.bat not found ***"; exit 1 }
...
foreach ($test in @("llvm","lld","eld","clang","polly")) {
    ...
    if (-not (ninja "check-$test")) {
        Write-Host "[ERROR] $test tests failed!"
        $FAIL_COUNT++
    }
}
